...
app.on("ready", () => {
    ...
    ipcMain.on("start-ble", async () => {
        ...
        python.on("close", (code) => {
            console.log(
                `Python measuring weight process exited with code ${code}`
            );
        });
    });
    ...
    ipcMain.on("start-face", (_event, base64Data: string) => {
        ...
        faceProcess.stdout.on("data", (data) => {
            try {
                ...
                if (message.type === "success") {
                    userState.set(
                        "race",
                        message.race === "AI" ? "asian" : "other"
                    );
                    userState.set("age", message.age);
                    userState.set(
                        "gender",
                        message.gender === "Man" ? "male" : "female"
                    );
        ...
        faceProcess.on("close", (code) => {
            console.log(
                `Python face analyzing process exited with code ${code}`
            );
        });
    });
                ...
                new Promise<void>((resolve, reject) => {
                    ...
                    const interval = setInterval(() => {
                        if (faceRecognitionDone) {
                            ...
                            reject(
                                new Error(
                                    "Timeout waiting for face recognition to complete"
                                )
                            );
                        }
                    }, pollInterval);
